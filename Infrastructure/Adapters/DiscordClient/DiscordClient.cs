...
internal class DiscordClient : IDiscordAccessTokenProvider, IDiscordLookupUser
{
    ...
    public async Task<User> Lookup(string accessToken)
    {
        ...
        var discordUserDto = await DeserializeContent<DiscordUserDto>(response);

        return discordUserDto.ToUser();
    }
